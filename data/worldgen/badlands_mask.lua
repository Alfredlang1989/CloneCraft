...
function sample(seed)
    local r=rain(); local t=temp(); local macro,continental=geology()
    local band=0.5+0.5*noise2(0.000095,621)
    local warm=smoothstep(0.48,0.72,t)
    local semidry=1.0-smoothstep(0.10,0.27,math.abs(r-0.36))
    local inland=smoothstep(0.43,0.72,continental)
    local nonpeak=1.0-smoothstep(0.70,0.84,macro)
    return clamp(math.sqrt(warm*semidry)*inland*nonpeak*(0.75+band*0.30)*land_factor(),0,1)
end
