...
function sample(seed)
    ...
    local patch=0.5+0.5*noise2(0.000090,604)
    local hot=smoothstep(0.52,0.76,t)
    local dry=1.0-smoothstep(0.30,0.52,r)
    local arid=math.sqrt(hot*dry)
    local nonmountain=1.0-smoothstep(0.60,0.76,macro)
    return clamp(arid*nonmountain*(0.82+patch*0.28)*1.15*land_factor(),0,1)
end
