...
OpenSimplex2S::OpenSimplex2S(std::uint64_t seed)
{
  short source[PSIZE] {};
  ...
  // The reference implementation intentionally uses Java long wraparound.
  // Signed overflow is UB in C++, so keep the LCG in uint64_t (defined modulo
  // 2^64), then bit-cast only for Java-compatible signed remainder semantics.
  std::uint64_t state = seed;
  for (int i = PSIZE - 1; i >= 0; i--) {
    state = state * UINT64_C(6364136223846793005) + UINT64_C(1442695040888963407);
    const std::int64_t signedForR = std::bit_cast<std::int64_t>(state + UINT64_C(31));
    int r = static_cast<int>(signedForR % (i + 1));
    if (r < 0) {
      r += (i + 1);
    }
    perm[i] = source[r];
...
double OpenSimplex2S::noise2_Base(double xs, double ys)
{
  ...
  const std::int64_t xsb = fastFloor(xs);
  const std::int64_t ysb = fastFloor(ys);
  double xsi = xs - xsb;
  double ysi = ys - ysb;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    const int pxm = ( static_cast<int>( xsb & PMASK ) + c.xsv ) & PMASK;
    const int pym = ( static_cast<int>( ysb & PMASK ) + c.ysv ) & PMASK;
 ...
 */
double OpenSimplex2S::noise2_Lattice(double xs, double ys)
{
  return noise2_Base(xs, ys);
}

double OpenSimplex2S::noise3_BCC_Lattice(double xr, double yr, double zr)
{
  return noise3_BCC(xr, yr, zr);
}
...
double OpenSimplex2S::noise3_BCC(double xr, double yr, double zr)
{
  ...
  const std::int64_t xrb = fastFloor(xr);
  const std::int64_t yrb = fastFloor(yr);
  const std::int64_t zrb = fastFloor(zr);
  double xri = xr - xrb;
  double yri = yr - yrb;
  double zri = zr - zrb;
  ...
  while (c != nullptr) {
    ...
    if (attn < 0) {
      ...
      const int pxm = ( static_cast<int>( xrb & PMASK ) + c->xrv ) & PMASK;
      const int pym = ( static_cast<int>( yrb & PMASK ) + c->yrv ) & PMASK;
      const int pzm = ( static_cast<int>( zrb & PMASK ) + c->zrv ) & PMASK;
...
double OpenSimplex2S::noise4_Base(double xs, double ys, double zs, double ws)
{
  ...
  const std::int64_t xsb = fastFloor(xs);
  const std::int64_t ysb = fastFloor(ys);
  const std::int64_t zsb = fastFloor(zs);
  const std::int64_t wsb = fastFloor(ws);
  double xsi = xs - xsb;
  double ysi = ys - ysb;
  double zsi = zs - zsb;
  double wsi = ws - wsb;
  ...
  const int index = static_cast<int>(
      ((fastFloor(xs * 4) & 3) << 0) | ((fastFloor(ys * 4) & 3) << 2) |
  ...
  for (int i = 0; i < LOOKUP_4D_SIZE[index]; ++i) {
    ...
    if (attn > 0) {
      ...
      const int pxm = ( static_cast<int>( xsb & PMASK ) + c.xsv ) & PMASK;
      const int pym = ( static_cast<int>( ysb & PMASK ) + c.ysv ) & PMASK;
      const int pzm = ( static_cast<int>( zsb & PMASK ) + c.zsv ) & PMASK;
      const int pwm = ( static_cast<int>( wsb & PMASK ) + c.wsv ) & PMASK;
...
std::int64_t OpenSimplex2S::fastFloor(double x)
{
  // World coordinates are 64-bit. The previous int return type silently
  // narrowed lattice coordinates and became undefined once a scaled noise
  // coordinate crossed INT32_MAX (about 54 billion blocks with the current
  // worldgen defaults). Keep the lattice coordinate at full engine width.
  if (!std::isfinite(x)) {
    throw std::domain_error("OpenSimplex2S requires finite coordinates");
  }
  ...
  if (x < kInt64Lower || x >= kInt64UpperExclusive) {
    throw std::overflow_error("OpenSimplex2S lattice coordinate exceeds int64 range");
  }

  const std::int64_t xi = static_cast<std::int64_t>(x);
...
void OpenSimplex2S::initLatticePoints()
{
  ...
  // The original C++ port allocated the 3D lookup chains with `new` and
  // intentionally leaked them for process lifetime. Keep the same pointer
  // topology in static storage instead: zero heap traffic, no shutdown leak.
  static LatticePoint3D lookup3DStorage[8][14] {};
  ...
  for (int i = 0; i < 8; i++) {
    ...
    LatticePoint3D* c0 = &(lookup3DStorage[i][0] = LatticePoint3D(i1, j1, k1, 0));
    LatticePoint3D* c1 = &(lookup3DStorage[i][1] = LatticePoint3D(i1 + i2, j1 + j2, k1 + k2, 1));
    LatticePoint3D* c2 = &(lookup3DStorage[i][2] = LatticePoint3D(i1 ^ 1, j1, k1, 0));
    LatticePoint3D* c3 = &(lookup3DStorage[i][3] = LatticePoint3D(i1, j1 ^ 1, k1 ^ 1, 0));
    LatticePoint3D* c4 = &(lookup3DStorage[i][4] = LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + k2, 1));
    LatticePoint3D* c5 = &(lookup3DStorage[i][5] = LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + (k2 ^ 1), 1));
    LatticePoint3D* c6 = &(lookup3DStorage[i][6] = LatticePoint3D(i1, j1 ^ 1, k1, 0));
    LatticePoint3D* c7 = &(lookup3DStorage[i][7] = LatticePoint3D(i1 ^ 1, j1, k1 ^ 1, 0));
    LatticePoint3D* c8 = &(lookup3DStorage[i][8] = LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + k2, 1));
    LatticePoint3D* c9 = &(lookup3DStorage[i][9] = LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + (k2 ^ 1), 1));
    LatticePoint3D* cA = &(lookup3DStorage[i][10] = LatticePoint3D(i1, j1, k1 ^ 1, 0));
    LatticePoint3D* cB = &(lookup3DStorage[i][11] = LatticePoint3D(i1 ^ 1, j1 ^ 1, k1, 0));
    LatticePoint3D* cC = &(lookup3DStorage[i][12] = LatticePoint3D(i1 + i2, j1 + j2, k1 + (k2 ^ 1), 1));
    LatticePoint3D* cD = &(lookup3DStorage[i][13] = LatticePoint3D(i1 + (i2 ^ 1), j1 + (j2 ^ 1), k1 + k2, 1));
