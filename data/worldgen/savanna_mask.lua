...
function sample(seed)
    local r=rain(); local t=temp(); local macro=geology()
    local patch=0.5+0.5*noise2(0.000090,651)
    local warm=smoothstep(0.50,0.72,t)
    local wetEnough=smoothstep(0.27,0.40,r)
    local notWet=1.0-smoothstep(0.50,0.64,r)
    local seasonal=wetEnough*notWet
    local lowland=1.0-smoothstep(0.60,0.76,macro)
    return clamp(math.sqrt(warm*seasonal)*lowland*(0.72+patch*0.20)*land_factor(),0,1)
end
